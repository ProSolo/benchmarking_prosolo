...
samples <- c("PAG1", "PAG2", "PAG5", "PAG9", "PAG10", "PNG")
...
for (sample in samples) {
  file <- str_c( sample, ".bps.SureSelectHumanAllExonV5plusUTR.target.mosdepth.region.dist.txt")
...
linetype_values = c(3, 2, 1, 4, 5, 6)
color_palette <- c(brewer.pal(name = "Set1", n = 7)[1:5], brewer.pal(name = "Set1", n = 7)[7])
...
ggplot(coverage_dists %>% filter(region == "total") %>% filter(coverage != 0), aes(x = coverage, y = fraction) ) +
  ...
  labs(x = "site coverage",
       y = "fraction of exome sites",
       tag = "B",
       title = "granulocytes, whole exome")
ggsave("Laehnemann2017_coverage_dist.pdf", device = cairo_pdf, width=10, height=6)

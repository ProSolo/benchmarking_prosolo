...
def printResults(outf, resultmatrix, nmRange, kRange, TRIALS):
    outf.write("Testing Missing Data\nk,");
...
def runTests():
    ...
    for f in ("perfectphy", "ms", "multextract.pl", "missingdata"):
        if not os.path.isfile(f):
            print 'Cannot find file "%s" in current directory' % f
            return
    
    nmRange = [ (x,1000) for x in xrange(10,101,10) ]
    kRange = [ (4,x) for x in ["0.0", "0.1", "0.2"] ]
    
    # create matrix of results for every (N,M,K) triplet
    resultmatrix = dict( [ ((n,m,k,md), Result()) for n,m in nmRange for k,md in kRange ] )
        ...
        for k, md in kRange:
            for n,m in nmRange:
                ...
                for t in xrange(1,TRIALS+1,PARALLEL):
                    ...
                    kws = [ subprocess.Popen("TIMEFORMAT='%%3U' ; ./missingdata -add %s < %dstate%d.%d.%d | (time %s)" % (md, k, n, m, ext, EXEC),

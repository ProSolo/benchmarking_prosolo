...
#export GRB_LICENSE_FILE=/path/to/gurobi_licence/gurobi.lic
LOGS=sge_logs/${#RUNS[@]}/
...
#snakemake --cluster "qsub -V -cwd -pe smp {threads} -l h_vmem={resources.mem_gb}G -q compute.q -o ./$LOGS -e ./$LOGS" --use-conda --output-wait 31 --stats $STATS --cores 24 2> >(tee $STDERR >&2)
nice snakemake --use-conda --conda-frontend mamba --resources mem_gb=300 --output-wait 47 --stats $STATS --cores 40 2> >(tee $STDERR >&2)

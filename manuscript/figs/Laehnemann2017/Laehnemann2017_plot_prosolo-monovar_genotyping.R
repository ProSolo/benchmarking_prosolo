...
SCs <- c("PAG1","PAG2", "PAG5", "PAG9", "PAG10")
...
for ( sc in SCs ) {
  for (f in fdrs ) {
    file <- str_c("prosolo/fdr_alt-presence/", sc,".PNG.alt_sites_only.fdr_", f , "_alt-presence.prosolo.min_sc_cov_1.gt_matching.ML_set.tsv")
...
for ( sc in SCs ) {
  file <- str_c("monovar/", sc,".single_cells.1.c0.monovar.gt_matching.tsv")
  current <- as_tibble( read_tsv( file ) ) %>%
              add_column(cell = sc,
...
ggplot(counts %>% filter(PED_GT != "0/0" & PED_GT != "not called"), aes( x = interaction(software, filter, sep = ", "), y = n )) +
  ...
  guides( 
          ...
          fill = "none" #guide_legend("ground truth matching", order = 2)
         ) +
...
ggsave("Laehnemann2017_genotyping_recall_prosolo_monovar.pdf", width=6, height=5)
...
ggplot(counts %>% filter(GT != "not called" & GT != "0/0"), aes( x = interaction(software, filter, sep = ", "), y = n )) +
  ...
  scale_x_discrete(limits = c("ProSolo, default, 0.2", "MonoVar, no cons, 1")) +
  scale_y_continuous(labels = comma)
ggsave("Laehnemann2017_genotyping_precision_prosolo_monovar.pdf", width=6, height=5)
...
ggplot(counts %>% filter(PED_GT != "0/0" & PED_GT != "not called"), aes( x = interaction(software, filter, sep = ", "), y = n )) +
  ...
  guides( 
          fill = "none" #guide_legend("caller genotype", order = 1)
         ) +
  scale_fill_manual(values =  gt_palette ) + #c(gt_palette[[4]], gt_palette[[1]], gt_palette[[2]] ) ) +
  scale_x_discrete(limits = c("ProSolo, default, 0.2", "MonoVar, no cons, 1")) +
  scale_y_continuous(labels = comma)
ggsave("Laehnemann2017_genotyping_calls_per_ground_truth_prosolo_monovar.pdf", width=6, height=5)
...
ggplot(counts %>% filter(GT != "not called" & GT != "0/0"), aes( x = interaction(software, filter, sep = ", "), y = n )) +
  ...
  guides( 
          fill = "none" #guide_legend("ground truth genotype", order = 1)
         ) +
  scale_fill_manual(values = gt_palette ) +
  scale_x_discrete(limits = c("ProSolo, default, 0.2", "MonoVar, no cons, 1")) +
  scale_y_continuous(labels = comma)
ggsave("Laehnemann2017_genotyping_ground_truth_per_calls_prosolo_monovar.pdf", width=6, height=5)
